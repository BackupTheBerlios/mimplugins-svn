...
/*
{
    ...
    if(DBGetContactSettingDword(hContact, szProto, "UIN", 0) == 1765432)
       _DebugPopup(0, "found foo");
}*/
...
{
    ChangeContactIcon((HANDLE) wParam, IconFromStatusMode((char*) CallService(MS_PROTO_GETCONTACTBASEPROTO, wParam, 0), ID_STATUS_OFFLINE, (HANDLE) wParam, NULL), 1);
    SortContacts();
    return 0;
}
...
int ContactSettingChanged(WPARAM wParam, LPARAM lParam)
{
    ...
    if (!DBGetContactSetting((HANDLE) wParam, "Protocol", "p", &dbv)) {
        if (!strcmp(cws->szModule, dbv.pszVal)) {
            ...
            if (!strcmp(cws->szSetting, "UIN") || !strcmp(cws->szSetting, "Nick") || !strcmp(cws->szSetting, "FirstName") || !strcmp(cws->szSetting, "LastName") || !strcmp(cws->szSetting, "e-mail")) {
                ...
                PostMessage(hwndContactTree, INTM_STATUSCHANGED, wParam, lParam);
                if (!DBGetContactSettingByte((HANDLE) wParam, "CList", "Hidden", 0)) {
                //if (!CLVM_GetContactHiddenStatus((HANDLE)wParam, cws->szModule)) {
    ...
    if (!strcmp(cws->szModule, "Protocol")) {
        if (!strcmp(cws->szSetting, "p")) {
            ...
            ChangeContactIcon((HANDLE) wParam, IconFromStatusMode(szProto, szProto == NULL ? ID_STATUS_OFFLINE : DBGetContactSettingWord((HANDLE) wParam, szProto, "Status", ID_STATUS_OFFLINE), (HANDLE) wParam, NULL), 0);
        }
    }
