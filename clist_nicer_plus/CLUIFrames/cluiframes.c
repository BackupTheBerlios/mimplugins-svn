...
int CLUIFramesStoreAllFrames()
{
    ...
    if (FramesSysNotStarted) return -1;
    if(g_shutDown) {
        _DebugPopup(0, "try to save frames while shutdown");
        return -1;
    }
    lockfrm();
    for (i=0;i<nFramescount;i++)
...
int CLUIFrameMoveResize(const wndFrame *Frame)
{
    ...
    if (Frame->TitleBar.ShowTitleBar) {
        //SetWindowPos(Frame->TitleBar.hwnd,NULL,Frame->wndSize.left + g_CluiData.leftOffset, Frame->wndSize.top + g_CluiData.topOffset -TitleBarH - GapBetweenTitlebar,
        //             (Frame->wndSize.right-Frame->wndSize.left) - g_CluiData.leftOffset,
        //             TitleBarH + (Frame->UseBorder ? (Frame->collapsed ? 2 : 1) : 0), SWP_NOZORDER|SWP_NOREDRAW);
...
int CLUIFramesOnClistResize(WPARAM wParam,LPARAM lParam)
{
   ...
   // nRect.top--;
    ...
    if (GetTickCount()-LastStoreTick>2000) {
        CLUIFramesStoreAllFrames();LastStoreTick=GetTickCount();
    };
                ...
                if (wParam&MK_LBUTTON) {
                    ...
                    if (Frames[pos].prevvisframe!=-1) {
                        ...
                        if (newh>0) {
                            ...
                            if (!CLUIFramesFitInSize()) {
                                Frames[Framemod].height=prevold; ulockfrm();return TRUE;
                            }
                            Frames[Framemod].height=newh;
