...
void SFL_Update(HICON hIcon, int iIcon, HIMAGELIST hIml, const char *szText)
{
	...
	if(!hIcon) {
		HICON p_hIcon = ImageList_ExtractIcon(0, hIml, iIcon);
		DrawIconEx(g_SFLCachedDC, 5, (szDest.cy - 16) / 2, p_hIcon, 16, 16, 0, 0, DI_NORMAL);
		DestroyIcon(p_hIcon);
	}
	else
		DrawIconEx(g_SFLCachedDC, 5, (szDest.cy - 16) / 2, hIcon, 16, 16, 0, 0, DI_NORMAL);

	hOldFont = SelectObject(g_SFLCachedDC, GetStockObject(DEFAULT_GUI_FONT));
	SetBkMode(g_SFLCachedDC, TRANSPARENT);
	GetTextExtentPoint32A(g_SFLCachedDC, szText, lstrlenA(szText), &szT);
	TextOutA(g_SFLCachedDC, 24, (szDest.cy - szT.cy) / 2, szText, lstrlenA(szText));
	SelectObject(g_SFLCachedDC, hOldFont);
	UpdateLayeredWindow(g_hwndSFL, 0, &ptDest, &szDest, g_SFLCachedDC, &ptSrc, GetSysColor(COLOR_3DFACE), &bf, ULW_ALPHA | ULW_COLORKEY);
}
...
void SFL_Create()
{
	...
	SetWindowPos(g_hwndSFL, HWND_TOPMOST, rcWindow.left, rcWindow.top, lWidth, min(20, sz.cy + 4), SWP_SHOWWINDOW);
	GetWindowRect(g_hwndSFL, &rcWindow);
