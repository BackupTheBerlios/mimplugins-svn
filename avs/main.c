...
HANDLE hProtoAckHook = 0, hContactSettingChanged = 0, hEventChanged = 0;
HICON g_hIcon = 0;
...
int CreateAvatarInCache(HANDLE hContact, struct avatarCacheEntry *ace, int iIndex, char *szProto)
{
    ...
    else if(hContact != 0){                     // don't update for pseudo avatars...
        UpdateAvatar(hContact);
        return -1;
    }
    return -1;
...
static int FindAvatarInCache(HANDLE hContact)
{
    ...
    if(i == g_curAvatar) {
        ...
        g_avatarCache = realloc(g_avatarCache, sizeof(struct avatarCacheEntry) * g_curAvatar);
...
static int ModulesLoaded(WPARAM wParam, LPARAM lParam)
{
    ...
    g_ProtoPictures = (struct protoPicCacheEntry *)malloc(sizeof(struct protoPicCacheEntry) * 50);
    ZeroMemory((void *)g_ProtoPictures, sizeof(struct protoPicCacheEntry) * 50);
...
static int DrawAvatarPicture(WPARAM wParam, LPARAM lParam)
{
    ...
    if(ace) {
		...
		if((r->dwFlags & AVDRQ_DRAWBORDER) && !((r->dwFlags & AVDRQ_HIDEBORDERONTRANSPARENCY) && (ace->dwFlags & AVS_HASTRANSPARENCY))) {
            ...
            HBRUSH brOld = SelectObject(r->hTargetDC, br);
