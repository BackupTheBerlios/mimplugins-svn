...
static int RoomWndResize(HWND hwndDlg,LPARAM lParam,UTILRESIZECONTROL *urc)
{
	...
	switch(urc->wId) {
		...
		case IDC_CHAT_LOG:
			...
			urc->rcItem.bottom = bToolbar?(urc->dlgNewSize.cy - si->iSplitterY - 24):(urc->dlgNewSize.cy - si->iSplitterY - 2);
...
{ 
	...
	mwdat = (struct MessageWindowData *)GetWindowLong(GetParent(hwnd),GWL_USERDATA);
    Parentsi = mwdat->si;
    ...
    switch (msg) {
        case WM_NCCALCSIZE:
            {
                LRESULT orig = CallWindowProc(OldMessageProc, hwnd, msg, wParam, lParam);
...
{ 
    ...
    case WM_NCCALCSIZE:
        {
            LRESULT orig = CallWindowProc(OldLogProc, hwnd, msg, wParam, lParam);
            NCCALCSIZE_PARAMS *nccp = (NCCALCSIZE_PARAMS *)lParam;
            BOOL bReturn = FALSE;
...
/*
			...
			if (wParam == VK_RETURN )
			{
				...
				item = SendMessage(GetDlgItem(GetParent(hwnd), IDC_LIST), LB_GETCURSEL, 0, 0);
				if(item != LB_ERR)
				{
...
BOOL CALLBACK RoomWndProc(HWND hwndDlg,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	{	
...
#ifdef _DEBUG
            _DebugTraceA("subclassed: %x", OldMessageProc);
#endif            
...
#ifdef _DEBUG
            _DebugTraceA("WM_INITDIALOG create window as tab #%d", dat->iTabID);
#endif
		} break;
		...
		{	
            ...
            /*
            x += GetTextPixelSize(pszDispName, (HFONT)SendMessage(si->hwndStatus,WM_GETFONT,0,0), TRUE);
			...
			SendMessageA(si->hwndStatus, SB_SETTEXTA,0,(LPARAM)pszDispName);
//			if(si->pszStatusbarText)
//			{
				SendMessageA(si->hwndStatus, SB_SETTEXTA,1,(LPARAM)(si->pszStatusbarText?si->pszStatusbarText:""));
				SendMessageA(si->hwndStatus, SB_SETTIPTEXTA,1,(LPARAM)(si->pszStatusbarText?si->pszStatusbarText:""));
//			}*/
			return TRUE;
		} break;
